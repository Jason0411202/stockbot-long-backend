...
- **API wire keys 不可變。** `internal/dto` 的 JSON tag 是前端既有契約（含唯一的 camelCase
  ...
  （`dto/equity.go` 的 `LiveEquityPoint`）回傳實盤每日權益時間序列供歷史折線圖。再新增 `/api/get_performance_history`
  （`dto/performance_history.go` 的 `PerformanceHistoryPoint`）：一條統一日期軸同時帶回測 + 實盤兩組逐日指標,
  實盤欄位以 `*float64` 承載（go-live 前為 `null`）;已/未實現損益不另存,由 `EquityHistory.cost_basis` 以
  `unrealized = 持股市值 − 成本基礎`、`realized = (總權益 − 投入本金) − unrealized` 還原。引擎為此新增唯讀的
...
- **EquityHistory 每日權益快照。** 線上引擎 catch-up 回放與每日 loop 都會以 `date` 為 PK upsert（`RecordEquity`）
  一筆當日權益（現金 + 持股市值 + 持倉成本基礎 `cost_basis`）到 `EquityHistory` 表，供 `/api/get_equity_history`
  與 `/api/get_performance_history` 歷史折線圖。寫入失敗僅警告、不致命，
